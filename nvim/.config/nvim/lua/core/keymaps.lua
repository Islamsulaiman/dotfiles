...
-- =====================================================================
-- SIMPLE UNTRACKED FILE INDICATORS
-- =====================================================================
...
-- Set up highlight group for untracked line numbers
vim.api.nvim_set_hl(0, 'UntrackedLineNr', { fg = '#ff9e64', bold = true })

-- Simple function to apply orange line numbers for untracked files
local function apply_untracked_line_numbers()
  local filepath = vim.fn.expand('%:p')
  local untracked = is_file_untracked(filepath)
  
  if untracked then
    -- Change line number color for untracked files
    vim.wo.winhl = 'LineNr:UntrackedLineNr,CursorLineNr:UntrackedLineNr'
  else
    -- Reset to normal line numbers
    vim.wo.winhl = ''
  end
end

-- Apply line number highlighting when entering a buffer
vim.api.nvim_create_autocmd({ 'BufEnter', 'BufWritePost' }, {
  callback = apply_untracked_line_numbers,
  pattern = '*',
})
